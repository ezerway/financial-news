...
- **Slider Widgets (`GoldSlider.astro`, `FuelSlider.astro`, `GasSlider.astro`)**: Integrates `attachComparison` to render day-before change badges across product cards (Buy/Sell, Zone 1/Zone 2, and 12kg/45kg boxes) and summary table slides. `GoldSlider` presents 2 summary table slides (splitting 18 gold products into 9-item tables) and 3 items per non-summary slide (6 group slides of 3 item cards each). `GasSlider` chunks 34 province items into 6 1-row horizontal item strips per slide with full multi-line province titles and compact 12K & 45K price chips across 2 summary table slides (parts 1/2 and 2/2) with distinct text colors (`#f97316` for 45KG column/prices and `#cbd5e1`/`#ffffff` for 12KG) to prevent UI overflow and ensure visual consistency. Dedicated widget routes (`/gold/widget`, `/oil/widget`, `/gas/widget`, and `/en/*` counterparts) serve visual embeds scaled at 1080px × 1920px (9:16 vertical format) without outer borders or border radiuses (`border: none; border-radius: 0;`), with high-density canvas capture streams and video player outputs.
